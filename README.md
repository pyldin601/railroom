...
Wheel impacts now use all carriages, with separate direct and structural sound paths per wheel. Direct gain is 0.3^carriageDistance / (1 + wheelDistance/18); structural gain is 0.62^carriageDistance / sqrt(1 + wheelDistance/35), using wheel-to-seat distance in meters. Seat changes update both paths smoothly. Existing rolling and motor isolation are unchanged. Each impact adds the same clack recording through a 4 kHz high-pass filter and 500 ms reverb decay (35% wet addition), cached once per recording, controlled by “Impact · metal hiss” and the overall Wheel impacts slider. This is an artistic approximation of structure-borne sound, not measured rail propagation. The existing 128-impact-group budget remains, with two synchronized sample sources per group; the metal tail finishes independently and both sources stop on cancellation.
