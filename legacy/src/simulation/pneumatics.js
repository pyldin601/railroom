...
export function advanceAir(input, controls, dt, powered = true) {
  ...
  air.cylinder = Math.max(0, air.cylinder + delta);
  if (target === 0 && air.cylinder < 0.005) air.cylinder = 0;
  air.releaseFlow = Math.max(0, -delta / dt);
  let compressor = air.compressor;
  if (air.reservoir <= 7.2) compressor = true;
  if (air.reservoir >= 9) compressor = false;
  air.compressor = compressor && powered;
  air.reservoir = Math.max(
    0,
    Math.min(
      9,
      air.reservoir - Math.max(0, delta) * 0.45 + (air.compressor ? 0.16 : 0) * dt - 0.0005 * dt,
    ),
  );
  return air;
}
