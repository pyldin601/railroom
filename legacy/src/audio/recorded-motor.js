...
const DURATION = 0.18;
const HOP = 0.06;
export const grainWindow = (phase) =>
  phase <= 0 || phase >= 1 ? 0 : (1 - Math.cos(2 * Math.PI * phase)) / 3;
export function recordedPitchRate(speed) {
  return 0.68 + 1.12 * Math.min(1, Math.max(0, speed) / (120 / 3.6));
}
export function sourcePosition(_speed, duration) {
  ...
  return Math.max(0, Math.min(0.3, duration - DURATION * 1.8 - 0.03));
}
...
export function alignGrain(data, sampleRate, desired, previous, playbackRate = 1) {
  ...
  let best = Math.max(0, Math.min(limit, target)),
    score = -Infinity;
  const radius = Math.floor(0.006 * sampleRate);
  for (
    let candidate = Math.max(0, target - radius);
    candidate <= Math.min(limit, target + radius);
    candidate += 4
  ) {
    let dot = 0,
      energy = 0,
    ...
    for (let i = 0; i < 512; i += 8) {
      const a = data[candidate + i] || 0,
        b = data[reference + i] || 0;
      dot += a * b;
      energy += a * a;
      refEnergy += b * b;
    }
...
export function textureNormalization(data, sampleRate, offset) {
  const start = Math.floor(offset * sampleRate),
    end = Math.min(data.length, start + DURATION * sampleRate);
  let energy = 0,
    count = 0;
  ...
  return Math.max(0.5, Math.min(4, 0.1 / Math.sqrt(energy / Math.max(1, count) + 1e-12)));
}
...
export class RecordedMotor {
  ...
  start() {
    ...
    const bogies = this.profile.axles
      ? this.profile.axles(axles)
      : axles.filter((_, i) => i % 4 === 0 || i % 4 === 2);
    this.voices = bogies.map((axle, i) => {
      ...
      return {
        output,
        fade,
        nextTime: this.context.currentTime + 0.015 + i * 0.002,
        sources: new Set(),
        previous: null,
        rate: null,
        lastUpdate: null,
  ...
  update(state, controls, running) {
    ...
    const load = parameters
      ? parameters.load
      : running && !controls.brake && !controls.emergency
        ? Math.max(0, Math.min(1, controls.throttle || 0))
        : 0;
    const gain =
      (Math.pow(load, 0.75) *
        (this.profile.gain ?? 0.19) *
        (this.mixer.levels[this.profile.level ?? 'traction'] ?? 1)) /
      Math.sqrt(this.voices.length);
    const buffer = this.sample.buffer,
      data = buffer.getChannelData(0);
    for (const voice of this.voices) {
      voice.output.gain.setTargetAtTime(gain, now, 0.12);
      if (this.profile.lazy && load === 0) {
        voice.rate = null;
        voice.previous = null;
        voice.nextTime = now + 0.015;
        continue;
      }
      ...
      voice.rate =
        voice.rate == null
          ? targetRate
          : voice.rate + (targetRate - voice.rate) * (1 - Math.exp(-dt / 0.18));
      voice.lastUpdate = now;
      if (voice.nextTime < now) {
        voice.nextTime = now + 0.01;
        voice.previous = null;
      }
      while (voice.nextTime < now + 0.15) {
        const desired = this.profile.offset
          ? this.profile.offset(voice, buffer.duration)
          : sourcePosition(state.speed, buffer.duration);
        const offset =
          this.profile.align === false
            ? desired
            : alignGrain(data, buffer.sampleRate, desired, voice.previous, voice.rate);
        ...
        envelope.gain.setValueCurveAtTime(
          this.window.map((v) => v * normalizer),
          voice.nextTime,
          DURATION,
        );
  ...
  stop(at = this.context.currentTime) {
    for (const voice of this.voices) {
      ...
      for (const source of voice.sources) source.stop(at + 0.035);
