...
export function motorParameters(speed, throttle, braking = 0) {
  ...
  return {
    ...
    gear: 105 + v * 24,
    body: 26 + v * 3.8,
    deviation: (10 + v * 1.4) * (0.2 + load * 0.8),
    cutoff: 650 + v * 24 + load * 350,
    gain: Math.pow(load, 0.75) * (0.065 + 0.025 * Math.min(1, v / 12)),
  };
}
...
export class TractionMotor {
  ...
  start() {
    ...
    const wave = ctx.createPeriodicWave(
      new Float32Array(5),
      new Float32Array([0, 1, 0.22, 0.075, 0.025]),
    );
    // Two independently voiced bogies per carriage; existing spatial buses handle seats and solos.
    const bogies = (this.mixer.audibleAxles ?? this.mixer.axles).filter(
      (_, i) => i % 4 === 0 || i % 4 === 2,
    );
    for (const [i, axle] of bogies.entries()) {
      ...
      const deviation = ctx.createGain(),
        gearLevel = ctx.createGain(),
        bodyLevel = ctx.createGain();
      const driftDepth = ctx.createGain(),
        filter = ctx.createBiquadFilter();
      const gain = ctx.createGain(),
        fade = ctx.createGain();
      gearLevel.gain.value = 0.15;
      bodyLevel.gain.value = 0.32;
      driftDepth.gain.value = 0.65; // cents: subtle independent motor beating, not a vibrato effect
      drift.frequency.value = 0.173 + i * 0.037;
      filter.type = 'lowpass';
      filter.Q.value = 0.55;
      gain.gain.value = 0;
      ...
      filter
        .connect(gain)
        .connect(fade)
        .connect(this.mixer.locomotive?.input ?? this.mixer.emitters.get(`${axle.id}:center`).gain);
      // Two alternating voices crossfade held notes without fourteen running oscillators.
      const pwmMod = ctx.createOscillator(),
        pwmDepth = ctx.createGain(),
        pwmGain = ctx.createGain();
      pwmMod.frequency.value = 24;
      pwmDepth.gain.value = 35;
      pwmGain.gain.value = 0;
      pwmMod.connect(pwmDepth);
      pwmGain.connect(fade);
      const pwmBands = [PWM_CARRIERS[0], PWM_CARRIERS[0]].map((frequency) => {
        ...
        level.gain.value = 0;
        pwmDepth.connect(oscillator.frequency);
        oscillator.connect(level).connect(pwmGain);
  ...
  tune(voice, p, time, count, immediate = false) {
    ...
    const smooth = (param, value, seconds = 0.09) =>
      immediate ? param.setValueAtTime(value, time) : param.setTargetAtTime(value, time, seconds);
    ...
    smooth(voice.deviation.gain, p.deviation, 0.16);
    smooth(voice.filter.frequency, p.cutoff, 0.18);
    voice.gain.gain.setTargetAtTime(
      (p.gain * (this.mixer.levels.traction ?? 1)) / Math.sqrt(count),
      time,
      0.12,
    );
  }
  ...
  update(state, controls, running) {
    if (!this.started) return;
    const p = motorParameters(
      state.speed,
      running ? controls.throttle || 0 : 0,
      controls.brake || controls.emergency,
    );
    const pwm = pwmParameters(state.speed, controls, running, this.pwmStage);
    this.pwmStage = pwm.stage;
    for (const voice of this.voices) {
      const t = this.context.currentTime;
      const tune = (param, value) =>
        voice.needsInitialTune
      ...
      if (voice.pwmStage !== pwm.stage) {
        ...
        voice.pwmBands[voice.pwmActive].oscillator.frequency.setValueAtTime(
          PWM_CARRIERS[pwm.stage],
          t,
        );
        voice.pwmBands.forEach((band, i) =>
          band.level.gain.setTargetAtTime(i === voice.pwmActive ? 1 : 0, t, 0.012),
        );
        voice.pwmStage = pwm.stage;
      }
      voice.pwmGain.gain.setTargetAtTime(
        (pwm.gain * (this.mixer.levels[pwm.braking ? 'brake' : 'traction'] ?? 1)) /
          Math.sqrt(this.voices.length),
        t,
        0.12,
      );
  ...
  stop(at = this.context.currentTime) {
    for (const voice of this.voices) {
      ...
      voice.fade.gain.linearRampToValueAtTime(0, at + 0.03);
      voice.oscillators.at(-1).onended = () => {
        for (const node of voice.nodes) node.disconnect();
      };
      for (const oscillator of voice.oscillators) oscillator.stop(at + 0.035);
    }
    this.voices = [];
