...
test('operating markers are unique, sorted and explicitly estimated', () => {
  ...
  markers.forEach((m, i) => {
    ...
    assert.equal(m.status, 'estimated');
    if (i) assert.ok(m.position >= markers[i - 1].position);
  });
});
...
test('Boiarka electrical markers are paired and never enter the impact scheduler', () => {
  const pair = route.operatingMarkers.filter((m) => m.groupId === 'boiarka-neutral-section');
  assert.deepEqual(
    pair.map((m) => m.type),
    ['power_off', 'power_on'],
  );
