...
test('metal hiss rejects bass and retains a decaying reverb tail', () => {
  const rate = 48000;
  const tone = (f) =>
    Float32Array.from({ length: rate }, (_, i) => Math.sin((2 * Math.PI * f * i) / rate));
  const rms = (x) => Math.sqrt(x.slice(24000, 48000).reduce((s, v) => s + v * v, 0) / 24000);
  assert.ok(
    rms(metalHissSamples(tone(100), rate)) < rms(metalHissSamples(tone(8000), rate)) * 0.01,
  );
  ...
  assert.ok(energy(0.2, 0.3) > 0);
  assert.ok(energy(0.5, 0.6) < energy(0.2, 0.3) * 0.002);
