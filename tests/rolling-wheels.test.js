...
test('rolling uses eight distinct wheel feeds only in the occupied carriage',()=>{
 ...
 assert.ok(feeds.every(f=>f.rate>=.99&&f.rate<=1.01),'wheel pitch stays within ±1%');
 ...
 assert.ok(Math.abs(cents.reduce((sum,c)=>sum+c,0))<1e-8,'keep the average pitch centred');
...
test('rolling pitch stays fixed from low speed through maximum speed',async()=>{
 ...
 const parameter=()=>({value:1,setTargetAtTime(v){this.value=v;}});
 const node=()=>({frequency:parameter(),Q:parameter(),gain:parameter(),playbackRate:parameter(),connect(){return this;},start(){}});
 const context={currentTime:0,createGain:node,createBufferSource:node,createBiquadFilter:node};
 const axles=wheelsets(1),mixer={axles,occupied:1,motorMode:'recorded',levels:{rolling:1},emitters:new Map(axles.flatMap(a=>['left','right'].map(side=>[`${a.id}:${side}`,{gain:node()}])))};
 const bank={pool:kind=>kind==='rolling'?[{buffer:{duration:9.85},gain:1}]:[]};
 const rolling=new RollingLayers(context,bank,mixer);rolling.start();
