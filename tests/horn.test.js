...
test('horn becomes quieter as the listener moves away from the locomotive',()=>{
 assert.ok(hornDistanceGain(117.484)<.2);
...
test('horn has an audible decaying tail after the blast and retains headroom',()=>{
 const context={sampleRate:24000,createBuffer(channels,length,rate){const data=new Float32Array(length);return {duration:length/rate,getChannelData:()=>data};}};
 const buffer=hornBuffer(context),data=buffer.getChannelData(0);
 ...
 assert.ok(energy(3.7,3.9)<energy(2.3,2.5)*.01,'long tail fades naturally');
