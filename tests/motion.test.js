...
test('braking integrates exact stop without reversing',()=>{let {state}=advanceMotion({...initialState(),speed:20},{brake:1},30,ideal);close(state.position,200);close(state.speed,0);});
test('brake wins over throttle',()=>{let a=advanceMotion({...initialState(),speed:10},{throttle:1,brake:1},1,ideal);close(a.state.speed,9);});
