...
export class SpatialMixer{
 constructor(context,bank,axles){
  ...
  for(const axle of axles)for(const side of ['left','right','center']){
   const pan=context.createPanner();pan.panningModel='HRTF';pan.distanceModel='inverse';pan.refDistance=3;pan.rolloffFactor=.3;pan.positionX.value=side==='left'?-.76:side==='right'?.76:0;pan.positionY.value=-1.5;pan.positionZ.value=axle.offset;
 ...
 hit(event,when,generation){
  ...
  source.connect(gain).connect(this.emitters.get(`${event.wheelsetId}:${event.side}`).gain);
  const voice={source,gain,when,kind:event.kind,generation};this.voices.add(voice);
  source.onended=()=>{this.voices.delete(voice);source.disconnect();gain.disconnect();};source.start(start);source.stop(start+duration+.01);
  this.lastImpacts.set(`${event.wheelsetId}:${event.side}`,when);
 }
 stopVoice(v,at){try{v.gain.gain.cancelScheduledValues(at);v.gain.gain.setTargetAtTime(0,at,.003);v.source.stop(at+.015);}catch{}this.voices.delete(v);}
