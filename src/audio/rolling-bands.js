...
export function splitRolling(context,source,destination){
 const bands={};
 for(const [name,type] of [['rollingLow','lowpass'],['rollingHigh','highpass']]){
  const filters=[context.createBiquadFilter(),context.createBiquadFilter()];
  for(const filter of filters){filter.type=type;filter.frequency.value=ROLLING_CROSSOVER_HZ;filter.Q.value=20*Math.log10(Math.SQRT1_2);}
...
export function updateRollingBands(bands,levels,time){
 for(const [name,band] of Object.entries(bands))band.gain.gain.setTargetAtTime(levels[name]??1,time,.03);
}
export function disposeRollingBands(bands){for(const band of Object.values(bands)){for(const f of band.filters)f.disconnect();band.gain.disconnect();}}
